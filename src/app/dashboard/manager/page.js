  ...
  useEffect(() => {
    const fetchShifts = async () => {
      try {
        ...
        console.error('Failed to fetch manager shifts:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchOnDuty = async () => {
      try {
        ...
        console.error('Failed to fetch manager on-duty analytics:', err);
      }
    };
    if (user) fetchOnDuty();
  }, [user]);
  ...
  return (
    <AppShell>
      ...
      <div style={{ 
        ...
        <Card title="On Duty Now" subtitle="Staff currently clocked in">
          <div style={{ fontSize: '2.5rem', fontWeight: '800', margin: 'var(--space-md) 0' }}>{stats.onDuty}</div>
          <div style={{ display: 'flex', gap: 'var(--space-sm)', flexWrap: 'wrap' }}>
            <Badge variant="success">Active Shifts</Badge>
          </div>
        </Card>
